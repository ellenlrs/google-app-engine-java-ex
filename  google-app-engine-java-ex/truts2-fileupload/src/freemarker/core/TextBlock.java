...
public final class TextBlock extends TemplateElement {
	...
	/**
	 * Scans forward the nodes on the same line to see whether there is a deliberate left trim in effect. Returns true if
	 * the left trim was present.
	 */
	private boolean deliberateLeftTrim() {
		boolean result = false;
		for (TemplateElement elem = this.nextTerminalNode(); elem != null && elem.beginLine == this.endLine; elem = elem
				.nextTerminalNode()) {
	...
	/**
	 * Checks for the presence of a t or rt directive on the same line. Returns true if the right trim directive was
	 * present.
	 */
	private boolean deliberateRightTrim() {
		boolean result = false;
		for (TemplateElement elem = this.prevTerminalNode(); elem != null && elem.endLine == this.beginLine; elem = elem
				.prevTerminalNode()) {
			if (elem instanceof TrimInstruction) {
				TrimInstruction ti = (TrimInstruction) elem;
				if (!ti.left && !ti.right) {
				...
				if (ti.right) {
					...
					if (trim(openingPart).length == 0) {
						...
						if (trim(trailingPart).length == 0) {
							...
							for (TemplateElement te = this.nextTerminalNode(); te != null && te.beginLine == this.endLine; te = te
									.nextTerminalNode()) {
	...
	/*
	 * private String leftTrim(String s) { int i =0; while (i<s.length()) { if (!Character.isWhitespace(s.charAt(i)))
	 * break; ++i; } return s.substring(i); }
	 */
	...
	private boolean nonOutputtingType(TemplateElement element) {
		return (element instanceof Macro || element instanceof Assignment || element instanceof AssignmentInstruction
				|| element instanceof PropertySetting || element instanceof LibraryLoad || element instanceof Comment);
	}
